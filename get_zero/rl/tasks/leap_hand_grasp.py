...
from get_zero.rl.tasks.leap_hand_rot import LeapHandRot


class LeapHandGrasp(LeapHandRot):
    """
    Note: this has to be run with the CPU pipeline because it's using gym.get_env_rigid_contacts, which only works on the CPU pipeline and there doesn't seem to be an equivalent that works the same on the GPU pipeline.

    Modifications:
    - Inherit from LeapHandRot from this repo rather than LeapHandRot from original repo -> enables ability to load different hardware configurations of LEAP hand
    - Modifications to support multiple hardware configuratinos (such as importing fingertip indices)
    """
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture=None, force_render=None):
        ...
        self.saved_grasping_states = torch.zeros((0, self.dof_count + 7), dtype=torch.float, device=self.device) # additional 7 for object pose
        ...
        self.fingertip_indices = cfg["env"]["fingertip_indices"]
    ...
    def reset_idx(self, env_ids):
        ...
        if len(self.saved_grasping_states) >= self.cfg["env"]["grasp_cache_len"]:
            ...
            print(f'Wrote grasp cache to {name}')
            exit()
        ...
        # self.obs_buf[env_ids] = 0 # TODO: removed from original since this should have no effect since compute_observation should be called right after reset_idx
        self.rb_forces[env_ids] = 0
        self.at_reset_buf[env_ids] = 1

    def compute_reward(self, actions):
        def list_intersect(li, hash_num):
            # self.dof_count + 1 is the object index
            # self.fingertip_indices are fingertip index
            # return number of contact with obj_id
            obj_id = self.dof_count + 1
            query_list = [obj_id * hash_num + fingertip_index for fingertip_index in self.fingertip_indices]
        ...
        finger_pos = self.rigid_body_states[:, self.fingertip_indices, :3]
